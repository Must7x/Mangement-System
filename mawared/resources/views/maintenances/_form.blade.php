            ...
            <form method="POST" action="{{ $action }}">
                ...
                @if (! $maintenance)
                    <div class="form-group">
                        <label for="asset_id" class="form-label">{{ __('fields.device') }}</label>
                        <select name="asset_id" id="asset_id" required class="form-select" @disabled($assets->isEmpty())>
                            <option value="">{{ __('common.select_warehouse_device') }}</option>
                        ...
                        @if ($assets->isEmpty())
                            <p class="alert alert-warning" style="margin-top:0.75rem;margin-bottom:0;font-size:0.8rem;">
                                {{ __('messages.maintenances.no_warehouse_devices') }}
                            </p>
                        @endif
                    </div>
                @else
                    <div class="form-group">
                        <label class="form-label">{{ __('fields.device') }}</label>
                        <p style="margin:0;font-weight:600;">{{ $maintenance->asset?->name }} {{ __('common.em_dash') }} {{ $maintenance->asset?->serial_number }}</p>
                    </div>
                @endif

                <div class="form-group">
                    <label for="issue_description" class="form-label">{{ __('fields.issue_description') }}</label>
                    <textarea name="issue_description" id="issue_description" rows="3" required class="form-input"
                              placeholder="{{ __('fields.issue_description_placeholder') }}">{{ old('issue_description', $maintenance?->issue_description) }}</textarea>
                </div>

                <div class="form-group">
                    <label for="priority" class="form-label">{{ __('fields.priority') }}</label>
                ...
                <div class="form-group">
                    <label for="technician_name" class="form-label">{{ __('fields.technician_name') }}</label>
                    <input type="text" name="technician_name" id="technician_name"
                           value="{{ old('technician_name', $maintenance?->technician_name) }}"
                           required class="form-input" placeholder="{{ __('fields.technician_name_placeholder') }}">
                </div>

                <div class="form-group">
                    <label for="status" class="form-label">{{ __('fields.request_status') }}</label>
                ...
                <div class="form-group">
                    <label for="maintenance_start_date" class="form-label">{{ __('fields.maintenance_start_date') }}</label>
                ...
                <div class="form-group">
                    <label for="notes" class="form-label">{{ __('fields.notes') }}</label>
                    <textarea name="notes" id="notes" rows="2" class="form-input"
                              placeholder="{{ __('fields.notes_placeholder') }}">{{ old('notes', $maintenance?->notes) }}</textarea>
                </div>
