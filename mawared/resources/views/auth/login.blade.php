...
<head>
    ...
    <title>{{ __('auth.page_title') }} — {{ __('branding.app_short') }}</title>
    ...
    <div class="guest-layout">
        <div class="login-grid">
            <div class="login-hero">
                <x-app-logo style="width:7.5rem;height:auto;background:#fff;border-radius:0.75rem;padding:0.5rem;margin-bottom:1.25rem;box-shadow:0 8px 24px rgb(0 0 0 / 0.2);" />

                <p style="font-size:1.25rem;font-weight:800;letter-spacing:0.12em;color:var(--color-mtnima-gold);margin:0 0 0.5rem;">{{ __('branding.acronym') }}</p>

                <h1 style="font-size:1.35rem;font-weight:700;margin:0 0 0.75rem;line-height:1.5;opacity:0.95;">
                    {!! nl2br(e(__('branding.ministry_name_multiline'))) !!}
                </h1>
                ...
                <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:0.75rem;font-size:0.875rem;">
                    <li><i class="fa-solid fa-check" style="color:var(--color-mtnima-gold);margin-left:0.5rem;"></i> {{ __('auth.feature_inventory') }}</li>
                    <li><i class="fa-solid fa-check" style="color:var(--color-mtnima-gold);margin-left:0.5rem;"></i> {{ __('auth.feature_assignments') }}</li>
                    <li><i class="fa-solid fa-check" style="color:var(--color-mtnima-gold);margin-left:0.5rem;"></i> {{ __('auth.feature_permissions') }}</li>
                </ul>

                <p style="margin-top:2rem;font-size:0.7rem;opacity:0.6;line-height:1.5;">
                    {!! nl2br(e(__('branding.ministry_name_fr'))) !!}
                </p>
            </div>
            ...
            <div class="login-form-panel">
                ...
                <div style="text-align:center;margin-bottom:1.5rem;">
                    <x-app-logo style="width:4rem;height:auto;margin:0 auto 0.75rem;display:block;" />
                ...
                <div style="margin-top:1.5rem;padding:1rem;background:#f8fafc;border-radius:0.75rem;border:1px solid var(--color-border);font-size:0.75rem;color:var(--color-muted);">
                    <p style="font-weight:600;color:var(--color-text);margin:0 0 0.5rem;">{{ __('auth.demo_accounts_title') }}</p>
                    <p style="margin:0.2rem 0;"><code>{{ __('auth.demo_account_admin') }}</code></p>
                    <p style="margin:0.2rem 0;"><code>{{ __('auth.demo_account_supervisor') }}</code></p>
                    <p style="margin:0.2rem 0;"><code>{{ __('auth.demo_account_storekeeper') }}</code></p>
                    <p style="margin:0.5rem 0 0;">{{ __('auth.demo_password') }} <strong>{{ __('auth.demo_password_value') }}</strong></p>
                </div>
            </div>
        </div>
    </div>
