...
@section('content')
    <x-page-header
        :title="$asset->name"
        subtitle="عرض شامل لبيانات الجهاز وسجلاته"
    >
        <x-slot:actions>
    ...
    <div class="card" style="margin-bottom:1.25rem;">
        <div class="card-body">
            <div style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:1rem;">
                <div>
                    <p style="margin:0 0 0.35rem;font-size:0.8rem;color:var(--color-muted);">Asset 360</p>
    ...
    <div class="card" style="margin-bottom:1.25rem;">
        ...
        <div class="card-body">
            <dl style="display:grid;grid-template-columns:repeat(auto-fit,minmax(12rem,1fr));gap:1rem 1.5rem;margin:0;">
                <div>
                    <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">الاسم</dt>
                    <dd style="margin:0;font-weight:600;">{{ $asset->name }}</dd>
                </div>
                <div>
                    <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">الفئة</dt>
                    <dd style="margin:0;font-weight:600;">{{ $asset->type }}</dd>
                </div>
                <div>
                    <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">الرقم التسلسلي</dt>
                    <dd style="margin:0;"><span class="serial-badge">{{ $asset->serial_number }}</span></dd>
                </div>
                <div>
                    <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">الحالة الحالية</dt>
                    <dd style="margin:0;"><x-status-badge :status="$asset->status" /></dd>
                </div>
                <div>
                    <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">تاريخ التسجيل</dt>
                    <dd style="margin:0;font-variant-numeric:tabular-nums;">{{ $asset->created_at->format('Y/m/d') }}</dd>
                </div>
                <div>
                    <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">آخر تحديث</dt>
    ...
    <div class="card" style="margin-bottom:1.25rem;">
        ...
        <div class="card-body">
            @if ($asset->status === \App\Enums\AssetStatus::Active && $asset->assignment)
                <dl style="display:grid;grid-template-columns:repeat(auto-fit,minmax(12rem,1fr));gap:1rem 1.5rem;margin:0;">
                    <div>
                        <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">الموظف</dt>
                        <dd style="margin:0;font-weight:600;">{{ $asset->assignment->employee?->name ?? $asset->assignment->employee_name }}</dd>
                    </div>
                    <div>
                        <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">القسم</dt>
                        <dd style="margin:0;">{{ $asset->assignment->employee?->department?->name ?? $asset->assignment->department }}</dd>
                    </div>
                    <div>
                        <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">تاريخ الإسناد</dt>
            ...
            @elseif ($asset->status === \App\Enums\AssetStatus::Maintenance && $asset->openMaintenance)
                <dl style="display:grid;grid-template-columns:repeat(auto-fit,minmax(12rem,1fr));gap:1rem 1.5rem;margin:0;">
                    <div>
                        <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">وصف العطل</dt>
                        <dd style="margin:0;">{{ $asset->openMaintenance->issue_description }}</dd>
                    </div>
                    <div>
                        <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">الفني</dt>
                        <dd style="margin:0;font-weight:600;">{{ $asset->openMaintenance->technician_name }}</dd>
                    </div>
                    <div>
                        <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">الأولوية</dt>
                        <dd style="margin:0;"><span class="status-badge status-maintenance">{{ $asset->openMaintenance->priority->value }}</span></dd>
                    </div>
                    <div>
                        <dt style="font-size:0.75rem;color:var(--color-muted);margin-bottom:0.25rem;">تاريخ بدء الصيانة</dt>
                        <dd style="margin:0;font-variant-numeric:tabular-nums;">{{ $asset->openMaintenance->maintenance_start_date->format('Y/m/d') }}</dd>
                    </div>
                </dl>
            @else
                <p style="margin:0;color:var(--color-muted);">لا توجد عهدة نشطة أو صيانة مفتوحة على هذا الجهاز.</p>
            @endif
        </div>
    </div>
                        ...
                        <tr>
                            <td colspan="5">
                                <div class="empty-state">
                                    ...
                                    <p>لا يوجد سجل عهد لهذا الجهاز.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <p>لا يوجد سجل صيانة لهذا الجهاز.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
